    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'name') and self.ant.name == 'Queen':
                    if insect.real_queen == True:
                        return
                    self.ant = None
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if hasattr(self.ant.ant, 'name') and self.ant.ant.name == 'Queen':
                        if insect.real_queen == True:
                            return
                    self.ant.ant = None
                else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        try:
            self.place.ant.blocks_path is True
        except:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does no damage but has a lot of armor"""
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for i in range(len(lst)):
            lst[i].armor -= self.damage
            new_lst = [lst[i] for i in range(len(lst)) if lst[i].armor > 0]
            self.place.bees = new_lst
        self.place.remove_insect(self)
        # END Problem 6A
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        while next_behind != None:
            if next_behind.ant:
                if hasattr(next_behind.ant, 'container') and next_behind.ant.container:
                    if not next_behind.ant in self.rear_ants:
                        next_behind.ant.damage *= 2
                        self.rear_ants += [next_behind.ant]
                    if (next_behind.ant.ant) and (not next_behind.ant.ant in self.rear_ants):
                        next_behind.ant.ant.damage *= 2
                        self.rear_ants += [next_behind.ant.ant]
                elif not next_behind.ant in self.rear_ants:
                    next_behind.ant.damage *= 2
                    self.rear_ants += [next_behind.ant]
